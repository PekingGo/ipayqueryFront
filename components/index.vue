<template id="#index" >
	<div>
			...
			<div class="mui-slider point">
		  		<div class="mui-slider-group mui-slider-loop">
		  			<div class="mui-slider-item mui-slider-item-duplicate">
				    	<query-category :detail="item" v-for="item in category.list4"  v-bind:key="item.id"></query-category>
				    </div>
				    <div class="mui-slider-item" >
				    	<query-category :detail="item" v-for="item in category.list1"  v-bind:key="item.id"></query-category>
				    </div>
				    <div class="mui-slider-item" >
				    	<query-category :detail="item" v-for="item in category.list2"  v-bind:key="item.id"></query-category>
				    </div>
		  			<div class="mui-slider-item">
				    	<query-category :detail="item"  v-for="item in category.list3" v-bind:key="item.id"></query-category>
				    </div>
				    <!--支持循环，需要重复图片节点-->
				    <div class="mui-slider-item mui-slider-loop">
				    	<query-category v-for="item in category.list1" v-bind:detail="item" v-bind:key="item.id"></query-category>
				    </div>
		  		</div>
		...
		<!--<div id="slider" class="mui-slider">
				<div class="mui-slider-group mui-slider-loop">
					<div class="mui-slider-item" :news="news" v-for = "info in news" :key="info.id">
						<a :href="info.url">
							<img :src="info.style">
		...
		<a  class="mui-btn  mui-btn-outlined backOld" href="http://cha.zfzj.cn">去旧版</a>
	</div>
	...
	var index = {
		...
		data:function(){
			return {
				category:{
						list1:[
							{flurl:'index.html',flname:'银行卡BIN',flplaceholder:'请输入银行卡前6位数字',frurl:'',frname:'开户行联行号',frplaceholder:'请输入开户行名称或联行号',id:1},
							{flurl:'',flname:'POS机交易应答码',flplaceholder:'请输入应答码或提示语',frurl:'',frname:'支付牌照',frplaceholder:'请输入支付公司名称',id:2}
						],
						list2:[
							{flurl:'',flname:'手刷品牌黑名单',flplaceholder:'请输入品牌名称',frurl:'',frname:'收单机构号',frplaceholder:'请输入收单机构号或机构名称',id:3},
							{flurl:'index.html',flname:'银联MCC码行业扣率',flplaceholder:'请输入MCC码、经营类型或经营项目',frurl:'',frname:'招行无积分商户',frplaceholder:'请输入完整的15位商户号',id:4}
						],
						list3:[
							{flurl:'',flname:'银联认证外包服务机构',flplaceholder:'请输入公司名',frurl:'',frname:'地区代码',frplaceholder:'请输入地区名称',id:5},
...
#container{height: 820px; margin: 0 auto; }
div.search {    
...
.bar6 input { border: 2px solid #007aff; border-radius: 5px;  background: #ffffff; top: 0; right: 0; } 
.bar6 button { background: #007aff; border-radius: 0 5px 5px 0; width: 60px; top: 0; right: 0; } 
